...
// weitgehend identisch mit shopsoftware_csv.inc.php
// Anzahl Spalten
$array_count = 41;
...
//marke;
   ...
   for ($i = 0; $i < count($info); $i++) {
      ...
      while ($tmp = $this->db->getObject()) {
         if ($tmp) {
            $tmp->images = $this->db->queryAllObjects("SELECT image FROM #__articles_images WHERE parent_id = ".$tmp->id." ORDER BY sort");
            $data[] = $tmp;
         }
      }
      ...
      for ($d = 0; $d< count($data); $d++) {
         ...
         $variante .= $this->_lexToAscii($wt.$data[$d]->art_nr).$wt.$trenner;
         $variante .= $wt.$data[$d]->gtin.$wt.$trenner;
         $variante .= $wt.$data[$d]->mpn.$wt.$trenner;

         $variante .= $wt.$data[$d]->online.$wt.$trenner;
         $variante .= $wt.$this->_lexToAscii($data[$d]->mm_name1).$wt.$trenner;
         $variante .= $wt.$this->_lexToAscii($data[$d]->w_name1).$wt.$trenner;
         $variante .= $wt.$this->_lexToAscii($data[$d]->mm_name2).$wt.$trenner;
         $variante .= $wt.$this->_lexToAscii($data[$d]->w_name2).$wt.$trenner;
      ...
      if ($config->html == 'text') {
         $beschreibung = $this->_lexToAscii($this->_html2txt((\KANPAICLASSIC\Helper::truncate($info[$i]->beschreibung, 300))));
      }
      ...
      $csv .= $wt.$tt.$this->_lexToAscii($info[$i]->name).$tt.$wt.$trenner;
      $csv .= $wt.$tt.$beschreibung.$tt.$wt.$trenner;
      $csv .= $wt.$info[$i]->cat_id.$wt.$trenner;
      $csv .= $wt.$this->_lexToAscii($info[$i]->catname).$wt.$trenner;
...
// deu_im Link      $csv .= $wt.$shopurl.(defined('CONF_USE_HTACCESS') ? '' : '/index.php').'/deu_'.$info[$i]->artikel_id.'/'.urldecode($info[$i]->name).$wt.$trenner;
      ...
      for ($p = 0; $p < 10; $p++) {
         $csv .= $wt.$this->_checkPict((isset($info[$i]->images[$p]) ? $info[$i]->images[$p]->image : ''), $picurl).$wt.$trenner;
      }
