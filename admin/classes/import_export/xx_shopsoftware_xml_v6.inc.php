...
if ($mode == 'export') {
   ...
   $sql = "SELECT i.id, i.cat_id, i.steuersatz, i.name_$lang AS name, i.desc_$lang AS beschreibung, $pics i.sortierung,
...
// deu_im Link      $xml .= '   <link><![CDATA['.$shopurl.(defined('CONF_USE_HTACCESS') ? '' : '/index.php').'/deu_'.$info[$i]->artikel_id.'/'.urldecode($info[$i]->name).']]></link>'.CR;
      ...
      for ($p = 1; $p <= (int)$this->params->firma['count_pics']; $p++) {
         $xml .= '   <bildlink'.$p.'>'.$this->_checkPicExport($info[$i]->{'pict'.sprintf('%02d', $p)}, $picurl).'</bildlink'.$p.'>'.CR;
      }
      ...
      for ($d = 0; $d< count($data); $d++) {
         ...
         $xml .= '      <ge_menge>'.$data[$d]->ge_menge.'</ge_menge>'.CR;
         $xml .= '   </variant>'.CR;
      }
...
if ($mode == 'import') {
   ...
   if ((float)$version < 3) {
      include $this->params->filepath.'/admin/classes/import_export/shopsoftware_xml_v2.inc.php';
      return;
   }

   else if ((float)$version < 4) {
      include $this->params->filepath.'/admin/classes/import_export/shopsoftware_xml_v3.inc.php';
      return;
   }

   else if ((float)$version < 5) {
      include $this->params->filepath.'/admin/classes/import_export/shopsoftware_xml_v4.inc.php';
      return;
   }

   else if ((float)$version < 6) {
      include $this->params->filepath.'/admin/classes/import_export/shopsoftware_xml_v5.inc.php';
      return;
   }
      ...
      for ($i = 0; $i < $anzahl; $i++) {
         ...
         $artikel = array();
         $artikel['id']             = $art_id;
         $artikel['art_nr']         = (string)$this->db->escape($articles[$i]->art_nr);
         ...
         for ($p = 1; $p <= (int)$this->params->firma['count_pics']; $p++) {
            $artikel['pict'.sprintf('%02d', $p)] = $articles[$i]->{'bildlink'.$p};
         }

         $artikel['staffelung']     = (string)$articles[$i]->staffelung;
         $artikel['grundeinheit']   = (string)$articles[$i]->grundeinheit;
         $artikel['ge_netto_aktiv'] = (string)$articles[$i]->geaktiv;
         ...
         for ($v = 0; $v < count($variants); $v++) {
            ...
            $variant['ge_menge']       = (float)$variants[$v]->ge_menge;
