...
if ($mode == 'export') {      // 1. Zeile enthält Spaltennamen
   ...
   $sql = "SELECT i.id, i.steuersatz, i.name_$lang AS name, i.desc_$lang AS beschreibung,
                  ...
                  i.versand_preis, i.gewicht, i.widerruf, i. lieferfrist,
...
// deu_im Link      $xml .= '   <link>'.$shopurl.(defined('CONF_USE_HTACCESS') ? '' : '/index.php').'/deu_'.$info[$i]->artikel_id.'/'.urldecode($info[$i]->name).'</link>'.CR;
      $xml .= '   <link>'.$shopurl.(defined('CONF_USE_HTACCESS') ? '' : '/index.php').'/'.$info[$i]->artikel_id.'/'.urldecode($info[$i]->name).'</link>'.CR;
      $xml .= '   <bildlink>'.($info[$i]->pict01 == '' || $info[$i]->pict01 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict01.'.jpg').'</bildlink>'.CR;
      $xml .= '   <bildlink2>'.($info[$i]->pict02 == '' || $info[$i]->pict02 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict02.'.jpg').'</bildlink2>'.CR;
      $xml .= '   <bildlink3>'.($info[$i]->pict03 == '' || $info[$i]->pict03 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict03.'.jpg').'</bildlink3>'.CR;
      $xml .= '   <bildlink4>'.($info[$i]->pict04 == '' || $info[$i]->pict04 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict04.'.jpg').'</bildlink4>'.CR;
      $xml .= '   <bildlink5>'.($info[$i]->pict05 == '' || $info[$i]->pict05 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict05.'.jpg').'</bildlink5>'.CR;
      $xml .= '   <bildlink6>'.($info[$i]->pict06 == '' || $info[$i]->pict06 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict06.'.jpg').'</bildlink6>'.CR;
      $xml .= '   <bildlink7>'.($info[$i]->pict07 == '' || $info[$i]->pict07 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict07.'.jpg').'</bildlink7>'.CR;
      $xml .= '   <bildlink8>'.($info[$i]->pict08 == '' || $info[$i]->pict08 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict08.'.jpg').'</bildlink8>'.CR;
      $xml .= '   <bildlink9>'.($info[$i]->pict09 == '' || $info[$i]->pict09 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict09.'.jpg').'</bildlink9>'.CR;
      $xml .= '   <bildlink10>'.($info[$i]->pict10 == '' || $info[$i]->pict10 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict10.'.jpg').'</bildlink10>'.CR;
      $xml .= '   <bildlink11>'.($info[$i]->pict11 == '' || $info[$i]->pict11 == 'nopic.png' ? '' : $picurl.'/'.CONF_PICT_PATH.$info[$i]->pict11.'.jpg').'</bildlink11>'.CR;
...
if ($mode == 'import') {
   ...
   for ($i = 0; $i < count($articles); $i++) {
      ...
      $artikel['versand_preis']  = (float)$articles[$i]->versand;
      $artikel['gewicht']        = (float)$articles[$i]->gewicht;
