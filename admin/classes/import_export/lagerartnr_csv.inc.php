...
if ($mode == 'import') {
   ...
   for ($i = 0; $i <count($artikel); $i++) {
      ...
      $sql = "SELECT a.id, i.steuersatz
                                        ...
                                        LEFT JOIN #__articles_info AS i
                                            ON a.parent_id = i.id
                                        WHERE a.art_nr like '".$act_artikel[0]."'";

      //echo $sql;

      $test_arr = $this->db->queryAllObjects($sql);

      // echo "found ".$act_artikel[0]." ".count($test_arr);
      // var_dump($test_arr);
      // echo "\r\n\r\n\r\n";

      if($test_arr != null && count($test_arr)>1){

          // throw new Exception("artnr doppelt: ".$act_artikel[0]);
      ...
      if ($test != null) {
         ...
         $brutto   = (float)str_replace(',', '.', $act_artikel[2]);
         $menge    = (float)str_replace(',', '.', $act_artikel[3]);
         $netto    = $brutto / (1 + $steuer);

         $sql = "UPDATE #__articles SET
                                online         = '".($act_artikel[1] == 'y' ? 'y' : 'n')."',
                                netto          = '".$netto."',
                                menge          = '".$menge."'
                            WHERE id = '".$test->id."'";
